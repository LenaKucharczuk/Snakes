...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int nCmdShow)
{
    ...
    wc1.lpszClassName = szClassName;	//nazwa klasy
    RegisterClass(&wc1);
...
{
    double cm = START_POSITION_X_RIGHT;
...
void PaintSnake(HDC* hdc, RECT rect, BOOL visibility)
{
    ...
    if (visibility)
    {
        hPen = CreatePen(PS_SOLID, 1, RGB(255, 0, 0));	// czerwony?
...
boolean move(HWND anotherWindow, int messageID, RECT rect)		// false- gra przegrana
{
    ...
    if (x >= rect.right || y >= rect.bottom || y < rect.top)
    {
        return false;
    }
    else
